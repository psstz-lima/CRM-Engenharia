...
class DWGService {
     ...
     * Usa abordagem hÃ­brida: tenta LibreDWG, fallback para parser DXF nativo
     */
    async convertToSvg(filePath: string, documentId: string): Promise<ConversionResult> {
        ...
        console.log('Iniciando conversÃ£o DWG/DXF:', documentId);
        ...
        try {
            if (ext === '.dxf') {
                ...
                return { success: false, error: 'Formato nÃ£o suportado' };
            }
        } catch (error: any) {
     ...
     * Fallback para representaÃ§Ã£o simplificada se nÃ£o disponÃ­vel
     */
    private async convertDwgToSvg(dwgPath: string, outputPath: string, documentId: string): Promise<ConversionResult> {
        ...
        if (this.odaConverterPath) {
            try {
                ...
                console.error('Erro na conversÃ£o ODA:', error);
        ...
        try {
            ...
            console.log('LibreDWG nÃ£o disponÃ­vel, usando visualizaÃ§Ã£o simplificada');
    ...
    private async convertDwgToDxfWithOda(dwgPath: string, outputPath: string, documentId: string): Promise<ConversionResult> {
        // ODA requer pastas de entrada e saÃ­da
        ...
        try {
            ...
            if (dxfFile) {
                ...
                throw new Error('ODA nÃ£o gerou arquivo DXF');
            }
...
<svg xmlns="http://www.w3.org/2000/svg" 
    ...
    private entityToSvgNew(entity: any, blocks: any, updateBounds: (x: number, y: number) => void, layerColors: Record<string, string> = {}): string | null {
        ...
        switch (entity.type) {
            ...
            case 'DIMENSION': {
                // DimensÃµes - desenhar linhas de extensÃ£o e texto
            ...
            case '3DFACE':
            case 'SOLID': {
                // 3D Face / Solid - desenhar como polÃ­gono
                const corners = [
    ...
    private parseDxfSections(content: string): Record<string, string> {
        ...
        // Regex ajustado para tolerar espaÃ§os antes dos cÃ³digos
    ...
    private extractLayers(tablesSection: string): LayerInfo[] {
        ...
        const layers: LayerInfo[] = [];
        // Regex ajustado para tolerar espaÃ§os
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (i % 2 === 0) {
                ...
                if (currentCode === 0) {
                    ...
                    // Adicionar dado Ã  entidade atual
    ...
    private aciToHex(colorIndex: number): string {
        const aciColors: Record<number, string> = {
            ...
            9: '#C0C0C0', // Light Gray
            10: '#FF0000',
            // Adicionar mais cores conforme necessÃ¡rio
        };
    ...
    /**
     * Criar SVG placeholder para quando a conversÃ£o nÃ£o Ã© possÃ­vel
     */
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 800 600" width="100%" height="100%">
    ...
    <g transform="translate(400, 300)">
        ...
        <text y="80" text-anchor="middle" font-family="Arial" font-size="14" fill="#6b7280">
            PrÃ©-visualizaÃ§Ã£o IndisponÃ­vel
        </text>
        <text y="100" text-anchor="middle" font-family="Arial" font-size="12" fill="#9ca3af">
            Salvar como .DXF permite visualizaÃ§Ã£o nativa
        </text>
    ...
    private async generateThumbnail(svgPath: string, documentId: string): Promise<string | undefined> {
        // Para thumbnail, seria necessÃ¡rio sharp ou similar
        // Por ora, retornamos undefined
        return undefined;
    }

    /**
     * Extrair layers de um SVG jÃ¡ convertido
     */
