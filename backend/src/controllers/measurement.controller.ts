...
export class MeasurementController {
    ...
    // Summary status per contract item (based on measurements)
    static async itemStatusByContract(req: Request, res: Response) {
        try {
            const { contractId } = req.params;

            const items = await prisma.measurementItem.findMany({
                where: { measurement: { contractId } },
                select: {
                    contractItemId: true,
                    measurement: { select: { status: true, createdAt: true } }
                }
            });

            const priority: Record<string, number> = { DRAFT: 1, CLOSED: 2, APPROVED: 3 };
            const result: Record<string, { status: string; lastDate: string; count: number }> = {};

            for (const row of items) {
                const status = row.measurement?.status || 'DRAFT';
                const current = result[row.contractItemId];
                const currentPriority = current ? (priority[current.status] || 0) : 0;
                const nextPriority = priority[status] || 0;
                const lastDate = row.measurement?.createdAt?.toISOString() || '';

                if (!current || nextPriority >= currentPriority) {
                    result[row.contractItemId] = {
                        status,
                        lastDate,
    ...
    static async getById(req: Request, res: Response) {
        ...
        try {
            ...
            if (!measurement) return res.status(404).json({ error: 'MediÃƒÂ§ÃƒÂ£o nÃƒÂ£o encontrada' });
    ...
    static async create(req: Request, res: Response) {
        ...
        try {
            ...
            if (!periodStart || !periodEnd) {
                ...
                return res.status(400).json({ error: 'periodStart e periodEnd sÃƒÂ£o obrigatÃƒÂ³rios' });
            ...
            if (start > end) {
                ...
                return res.status(400).json({ error: 'periodStart nÃƒÂ£o pode ser maior que periodEnd' });
            ...
            await ContractEventService.log(
                ...
                `MediÃƒÂ§ÃƒÂ£o criada: #${measurement.number}`,
    ...
    static async updateItem(req: Request, res: Response) {
        ...
        try {
            ...
            if (!Number.isFinite(normalizedQuantity) || normalizedQuantity < 0) {
                ...
                return res.status(400).json({ error: 'Quantidade invÃƒÂ¡lida' });
    ...
    static async close(req: Request, res: Response) {
        ...
        try {
            ...
            await ContractEventService.log(
                ...
                `MediÃƒÂ§ÃƒÂ£o fechada: #${measurement.number}`,
            ...
            return res.json({ message: 'MediÃƒÂ§ÃƒÂ£o fechada com sucesso' });
